...
ALUResult ALU::execute(uint32_t operand_a, uint32_t operand_b, ALUControl control) {
    ALUResult res{};
    res.overflow = false;
    ...
    switch (control) {
        case ALUControl::ADD:
            ...
            res.overflow = detect_add_overflow(operand_a, operand_b, res.result);
            break;
        ...
        case ALUControl::SUB:
            res.result = operand_a - operand_b;
            res.overflow = detect_sub_overflow(operand_a, operand_b, res.result);
            break;
        ...
        case ALUControl::SLTU:
            res.result = (operand_a < operand_b) ? 1 : 0;
            break;
        default:
            res.result = 0;
            break;
